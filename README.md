...
로그 확인 중 가장 많은 Client IP가 WAF에 막혀서 403이 발생했던 것이고, 해당 IP는 CloudFront의 Edge Location 중 하나인 것이 확인됐다. <bk>
ALB에 적용되어 있는 WAF Rule에 우연히 새로 바뀐 CloudFront Edge Location IP 대역이 BlackList에 적용되어 있는 것이었고, 해당 Rule에서 제거하니 문제는 해결됐다. <br> 

# 조치 방안
Amazon은 내부 서비스 IP 대역을 주기적으로 변경한다. <br>
